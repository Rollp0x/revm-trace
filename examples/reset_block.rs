...
async fn main() -> Result<()> {
    println!(
        "{}",
        "🔍 Testing balance queries at different block heights with both backends\n"
            .cyan()
            .bold()
    );
    ...
    println!(
        "{}",
        "ℹ️  SharedBackend test skipped (foundry-fork feature not enabled)".yellow()
    );

    println!("{}", "💡 The balance difference includes both the transfer amount (0.09445674 ETH) and gas fees (~0.00115 ETH)".cyan());
    println!(
        "{}",
        "🚀 ResetBlock trait with associated Error type works correctly!".magenta()
    );
